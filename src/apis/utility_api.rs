...
pub enum HealthError {
    ...
    Status503(models::Health200Response),
...
pub async fn health(configuration: &configuration::Configuration, ) -> Result<models::Health200Response, Error<HealthError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Health200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Health200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn root(configuration: &configuration::Configuration, ) -> Result<models::Root200Response, Error<RootError>> {
    ...
    let uri_str = format!("{}/api/v3", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
